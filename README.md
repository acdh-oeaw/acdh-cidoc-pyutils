...
### creates events from tei:org|place|person

takes a tei:person|place|org node, extracts `event` and `event/time-span` texts, `@xml:lang` and custom type values and returns `cidoc:P2_has_type`, `cidoc:P7_took_place_at`, `cidoc:P4_has_time-span` and `cidoc:P4_has_time-span` nodes linked via `cidoc:P2_has_type`
...
for x in doc.xpath(".//tei:place|tei:org|tei:person|tei:bibl", namespaces=NSMAP):
    xml_id = x.attrib["{http://www.w3.org/XML/1998/namespace}id"].lower()
    event_type = x.xpath("./tei:event/@type")[0]
    item_id = f"https://foo/bar/{xml_id}/{event_type}"
    subj = URIRef(item_id)
    g.add((subj, RDF.type, CIDOC["E55_Type"]))
    g += make_events(
